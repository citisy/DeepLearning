...
from utils import torch_utils, configs
...
class Config(bundles.Config):
    ...
    def make_full_config(cls):
        return {
            ...
            'xl_mid': configs.ConfigObjParse.merge_dict(sdxl.Config.xl_base_backbone, dict(
                attend_layers=[2],
                transformer_depth=[0, 0, 1],
            )),

            'xl_small': configs.ConfigObjParse.merge_dict(sdxl.Config.xl_base_backbone, dict(
                attend_layers=[],
...
class ModelWrap:
    ...
    def wrap(self, model: nn.Module):
        ...
        control_model_kwargs = configs.ConfigObjParse.merge_dict(dict(
            in_ch=backbone.in_channels,
            hint_in_ch=3,
            context_dim=backbone.context_dim
        ), self.control_model_kwargs)
        control_model = ControlBlock(**control_model_kwargs)
        control_model = control_model.to(
            device=torch_utils.ModuleInfo.possible_device(backbone),
            dtype=torch_utils.ModuleInfo.possible_dtype(backbone)
        )
        backbone.control_model = control_model

        self.backbone_forward = backbone.forward  # used to restore
        backbone.forward = partial(UNetModel.forward, backbone)
...
class UNetModel(ldm.UNetModel):
    def forward(self, x, timesteps=None, context=None, y=None, control_images=None, only_mid_control=False, **kwargs):
        controls = self.control_model(x, control_images, timesteps, context, y=y)
...
class ControlBlock(nn.Module):
    def __init__(
            ...
            hint_out_ches=(16, 16, 32, 32, 96, 96, 256), hit_strides=(1, 1, 2, 1, 2, 1, 2),  # for input_hint_block
            **kwargs
    ):
        super().__init__()
        ...
        if num_classes is not None:
            self.label_emb = ldm.UNetModel.make_label_emb(self, num_classes, time_emb_dim, unit_dim, adm_in_channels)
