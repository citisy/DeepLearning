...
class ModelHooks:
    ...
    def __init__(self):
        ...
        self.train_start_container: List[list] = []
        self.train_end_container: List[list] = []
        self.val_start_container: List[list] = []
        self.val_end_container: List[list] = []

        def _load_checkpoint(
                load_checkpoint=False, load_checkpoint_include=None, load_checkpoint_exclude=None,
                **kwargs
        ):
            if load_checkpoint:
    ...
    def set_counter(self):
        self.counter = torch_utils.Counter()
    ...
    @staticmethod
    def register_container(container: List[list], func, name, insert_idx, **kwargs):
        name = name or func.__name__

        if insert_idx < 0:
            insert_idx = len(container) + insert_idx + 1

        container.insert(insert_idx, [name, func, kwargs])

    def register_train_start(self, func, name=None, insert_idx=-1, **kwargs):
        self.register_container(self.train_start_container, func, name, insert_idx, **kwargs)

    def register_train_end(self, func, name=None, insert_idx=-1, **kwargs):
    ...
    def on_train_start(
        ...
        for name, func, params in self.train_start_container:
            func(process_kwargs=process_kwargs, loop_objs=loop_objs, **kwargs, **params)
    ...
    def on_train(self, loop_objs, **kwargs):
        ...
        for i in range(self.counter.cur_epoch, max_epoch):
            ...
            self.counter.total_epoch_steps = len(train_dataloader)
    ...
    def on_train_epoch_start(self, loop_objs, **kwargs):
        self.counter.reset_epoch()
    ...
    def on_backward(self, loop_objs, accumulate=None, batch_size=None, use_ema=False, use_scaler=False, **kwargs):
        ...
        if accumulate:
            loss = loss / (accumulate / batch_size)
        ...
        if accumulate:
            if self.counter.cur_nums % accumulate < batch_size:
    ...
    def on_train_step_end(
        ...
        self.counter.update_step(len(loop_inputs))
        ...
        for k, v in model_results.items():
            if k.startswith('loss'):
                ...
                if ignore_non_loss and np.isnan(v):
                    ...
                    # count mean losses from each check period
                    loop_objs[n] = loop_objs.get(n, 0) + v
                    losses[k] = v
                    losses[f'mean_{k}'] = loop_objs[n] / self.counter.cur_period_nums
    ...
    def _check_on_train_step_end(
        ...
        if (
            ...
            state_dict = self._check_train(loop_objs, max_save_weight_num, self.counter.cur_nums, check_period_trace=check_period_trace, **kwargs)
            if is_metric:
    ...
    def _check_on_train_epoch_end(
        ...
        cur_epoch = self.counter.cur_epoch - 1  # cur_epoch will be increased before the check
        self.trace({'epoch': cur_epoch}, check_period_trace)
        if (
                (check_period and cur_epoch % check_period == check_period - 1)
                or (cur_epoch == max_epoch - 1 and self.counter.cur_epoch_steps == self.counter.total_epoch_steps)
        ):
    ...
    def _check_train(self, loop_objs, max_save_weight_num, check_num, check_period_trace=(bundled.LOGGING, bundled.WANDB), **kwargs):
        ...
        if losses is not None:
            ...
            # reset check vars each check period
            self.counter.reset_period()
            for k in loop_objs:
    ...
    def _check_metric(self, loop_objs, state_dict, check_num, max_save_weight_num, metric_kwargs=dict(), check_period_trace=(bundled.LOGGING, bundled.WANDB), **kwargs):
        results = self.metric(**metric_kwargs)
        scores = {}
    ...
    def on_train_end(self, **kwargs):
        for name, func, params in self.train_end_container:
            func(**params)
    ...
    def metric(self, *args, **kwargs) -> dict:
        ...
        must return a dict like {model_name: {"score": xxx}}"""
        raise NotImplementedError
    ...
    def register_val_start(self, func, name=None, insert_idx=-1, **kwargs):
        self.register_container(self.val_start_container, func, name, insert_idx, **kwargs)
    ...
    def on_val_start(self, val_data=None, val_dataloader=None, batch_size=None, load_checkpoint=False, data_get_kwargs=dict(), dataloader_kwargs=dict(), epoch=-1, total_nums=-1, **kwargs):
        ...
        for name, func, params in self.val_start_container:
    ...
    def batch_predict(self, *objs, total=None, vis_pbar=True, **kwargs):
        ...
        if vis_pbar:
            ...
            self.register_logger('predict_pbar', pbar.update)
        for i in range(0, total, batch_size):
            ...
            self.log(end_idx - start_idx, 'predict_pbar')
